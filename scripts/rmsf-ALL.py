...
def main ():
	...
	OUTFILE	   = "nonbonded-energies.csv"
	WORKINGDIR = os.getcwd()

	# Execute in parallel with multiple arguments
	pool = mp.Pool (maxtasksperchild=1, processes=2)
	dirList = os.listdir (inputDir)
	params = [inputDir, outputDir, PSFFILE, DCDFILE, WORKINGDIR]
	#pool.map (partial (calculateRMSFs, inputDir, outputDir, PSFFILE, DCDFILE, WORKINGDIR), dirList) 
	for dir in dirList:
		calculateRMSFs (inputDir, outputDir, PSFFILE, DCDFILE, WORKINGDIR, dir) 
		os.chdir (WORKINGDIR)

#---------------------------------------------------------------------
#---------------------------------------------------------------------
def calculateRMSFs (inputDir, outputDir, PSFFILE, DCDFILE,  WORKINGDIR, dir):
	...
	# Calculate
	cmm1 = "rmsf-trajectory.tcl %s %s" % (inPSF, inDCD)
