...
def main ():
	...
	#pool.map (partial (calculateRotTrasDCD, inputDir, psfFile), dcdsList) 
...
def calculateRotTrasDCD (inputDir, psfFile, dcdFile):
    cmm = "rottrans-trajectory.tcl %s/%s %s/%s" % (inputDir, psfFile, inputDir, dcdFile)
    print (cmm)
    os.system (cmm)
