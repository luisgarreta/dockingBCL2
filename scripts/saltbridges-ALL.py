...
def main ():
	...
	outputDir  = "out-saltbr"
	PSFFILE	   = "step5_input.psf"
	DCDFILE	   = "step7-DCDs.dcd"
	OUTFILE	   = "saltbr-trajectories.csv"
	WORKINGDIR = os.getcwd()

	# Execute in parallel with multiple arguments
	pool = mp.Pool (maxtasksperchild=1)
	dirList = os.listdir (inputDir)
	pool.map (partial (calculateSALTBR, inputDir, outputDir, PSFFILE, DCDFILE, WORKINGDIR), dirList) 
	#for dir in dirList:
	#    calculateSALTBR (inputDir, outputDir, PSFFILE, DCDFILE, WORKINGDIR, dir) 
	#    os.chdir (WORKINGDIR)


#---------------------------------------------------------------------
#---------------------------------------------------------------------
def calculateSALTBR (inputDir, outputDir, PSFFILE, DCDFILE,  WORKINGDIR, dir):
	...
	# Calculate
	cmm1 = "saltbridges-trajectory.tcl %s %s" % (inPSF, inDCD)
