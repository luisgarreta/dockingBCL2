...
proc rotationsTransfMatrix {nFrames OUTFILE} {
	set SEL "segid PROA and not resid 65 to 84 and not resid 208 to 231"
	...
	set f0 [atomselect top $SEL frame 0]

	for {set i 1} {$i < [expr {$nFrames-1}]} {incr i} {
		puts [format ">>>>>>>>>>>>>>> FRAME: %s <<<<<<<<<<<<<<<<<<" $i]
		set j [expr {$i+1}]
		set f1 [atomselect top $SEL frame $j]

		set T [measure fit $f0 $f1 weight mass]
    	# now $T holds in its columns the principal axes
    	set r11 [lindex [lindex $T 0] 0] 
    	set r12 [lindex [lindex $T 0] 1] 
    	set r13 [lindex [lindex $T 0] 2] 
    	set r21 [lindex [lindex $T 1] 0] 
    	set r22 [lindex [lindex $T 1] 1] 
    	set r23 [lindex [lindex $T 1] 2] 
    	set r31 [lindex [lindex $T 2] 0] 
    	set r32 [lindex [lindex $T 2] 1] 
    	set r33 [lindex [lindex $T 2] 2] 

		set Theta [expr acos ($r33) * $DEGPI]
		set Phi [expr atan ($r23/$r13) * $DEGPI]
		set Psi [expr atan ($r32/-$r31) * $DEGPI]
