...
# and calculate free binding energies
	...
	for trajdir in trajDirs:
		...
		framesDirs = [x for x in os.listdir (trajpath)]
	
		pool1 = mp.Pool (maxtasksperchild=1)
		pool1.map (createConfigFiles, framesDirs)

		pool2 = mp.Pool (maxtasksperchild=1)
		pool2.map (calculateBindingEnergies, framesDirs)
		os.chdir (workingDir)
...
def createConfigFiles (frameDir):
	...
	os.system ("ln -s %s-protein.pdb protein.pdb" % frameDir) 
	os.system ("ln -s %s-ligand.pdb ligand.pdb" % frameDir)
...
#----------------------------------------------------------------------
#----------------------------------------------------------------------
def calculateBindingEnergies (frameDir):
	workingDir = os.getcwd()
	os.chdir (frameDir)
