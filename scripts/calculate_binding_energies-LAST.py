...
#	allNamdis = ["%s/%s" % (namdisDir, x) for x in os.listdir (namdisDir)]
#	for namdDir in allNamdis:
#		print (">>>", namdDir)
#		ligandDir  = os.path.basename (namdDir).split ("-")[1]
#		configFiles = createConfigFiles (namdDir, ligandDir)
#		#calculateBindingEnergies (configFiles)
#
#	os.mkdir (energiesDir)
#	os.system ("mv conf* %s" % energiesDir)

	# Calculate binding energies
	configDirsAll = ["%s/%s" %(energiesDir,x) for x in os.listdir (energiesDir)]

	# Process config dir in parallel
	pool = mp.Pool ()
...
#----------------------------------------------------------------------
def createConfigFiles (namdDir, ligandDir):
	os.system ("mkdir %s" % ligandDir)
	os.system ("cp %s/frame-LAST-protH.pdb %s/protein.pdb" % (namdDir, ligandDir))
	os.system ("cp %s/frame-LAST-heta.pdb %s/ligand.pdb" % (namdDir, ligandDir))
