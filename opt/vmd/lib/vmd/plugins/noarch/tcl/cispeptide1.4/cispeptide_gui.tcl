...
#
# $Id: cispeptide_gui.tcl,v 1.8 2020/05/09 00:47:12 johns Exp $
#

package provide cispeptide_gui 1.4
...
proc ::cispeptide::GUI::cispeptide_gui { } {
  ...
  set lf [labelframe $w.check -bd 2 -relief ridge -text "Check protein structure for cis peptide bonds" -padx 1m -pady 1m] 
#  grid $lf -row $row -columnspan 3 -sticky nsew
#  incr row
  pack $lf -side top -fill x
...
#  incr irow 
  ...
  set lf [labelframe $w.list -bd 2 -relief ridge -text "Identified cis peptide bonds" -padx 1m -pady 1m] 
  pack $lf -side top -fill x
  ...
  labelframe $w.atoms -bd 2 -relief ridge -text "Select and move atoms in cis peptide bonds" -padx 1m -pady 1m

  set f [frame $w.atoms.frame]
  grid $f -row 0 -column 0 -sticky ew

  set lf [label $f.label -text "Tag atom to be moved for selected cis peptide bonds:"] 
  grid $lf -row 0 -column 0 -columnspan 3 -sticky ew

  grid [button $f.setbutton1 -text "hydrogen" -command "[namespace current]::cispeptideSet H"] -row 1 -column 0 -sticky ew
  grid [button $f.setbutton2 -text "oxygen" -command "[namespace current]::cispeptideSet O"] -row 1 -column 1 -sticky ew
  grid [button $f.setbutton3 -text "none" -command "[namespace current]::cispeptideSet X"] -row 1 -column 2 -sticky ew
  grid [button $f.movebutton -text "Move tagged atoms for selected cis peptide bonds" -command [namespace current]::cispeptideMove] -row 2 -column 0 -columnspan 3 -sticky ew
