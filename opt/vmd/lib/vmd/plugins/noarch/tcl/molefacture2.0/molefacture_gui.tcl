...
proc ::Molefacture::molefacture_gui { {sel ""} } {
   ...
   ### NEW export to qmtool
   $topGui.topframe.menubar.file.menu add command -label "Export to QMTool" -command ::Molefacture::export_qmtool
...
# closed                                             #
######################################################
proc ::Molefacture::closeMainWindow {args} {

   # If called from export_qmtool don't ask for confirmation. already done.
   if {$args != "export_qmtool"} {
...
proc Molefacture::checkCgenffUser {} {
  ...
  if {[winfo exists $Molefacture::cgenffWindow.mainframe.entry] == 1 && \
   ...
   $env(CGENFFUSERNAME) == ""} {
    ...
    tk_messageBox -message "Something went wrong. Please try again." -title "CGenFF Username" -icon error \
    -type ok -parent $Molefacture::cgenffWindow
  }
...
proc ::Molefacture::exportFile { tmpsel filename format} {
   ...
   ### The package on the "else" part of the statement is also 
   ...
   if {$Molefacture::cgenffcaller == 1} {
      if {[cgenffcaller savemol -sel $tmpsel -output $filename -format $format] == -1} {
