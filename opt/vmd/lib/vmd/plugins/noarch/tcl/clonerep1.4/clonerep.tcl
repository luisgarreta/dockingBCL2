...
########################################################################
#
# $Id: clonerep.tcl,v 1.15 2020/05/09 00:39:51 johns Exp $
#
...
namespace eval ::CloneRep:: {
    ...
    variable version   1.4;   # version number of this plugin
}
...
proc ::CloneRep::clone_reps {{fromid top} {toid all}} {
    ...
    if {$fromid == $toid} {
        if {[info exists w]} then {
            ...
            vmdcon -err "clone_reps: cannot clone a representation on itself."
        }
        return
    }
    if { [lsearch -exact [molinfo list] $fromid] < 0} {
        if {[info exists w]} then {
            ...
            vmdcon -err "clone_reps: source molecule id $fromid does not exist"
        }
        return
    }
    if { !$togroup && [lsearch -exact [molinfo list] $toid] < 0} {
        if {[info exists w]} then {
            ...
            vmdcon -err "clone_reps: target molecule id $toid does not exist"
        }
        return
    }
...
proc ::CloneRep::clonerepgui {} {
    ...
    # layout main canvas
    grid configure $w.menubar -column 0 -row 0 -columnspan 5 -sticky "snew"
    grid configure $w.from    -column 0 -row 1 -columnspan 2 -sticky "snew"
    grid configure $w.to      -column 3 -row 1 -columnspan 2 -sticky "snew"
    grid configure $w.foot    -column 0 -row 2 -columnspan 5 -sticky "snew"
    grid columnconfigure $w 0 -weight 2 -minsize 100
    grid columnconfigure $w 1 -weight 1 -minsize 100
