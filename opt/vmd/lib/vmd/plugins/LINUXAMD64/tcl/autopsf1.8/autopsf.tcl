...
##
##           
## $Id: autopsf.tcl,v 1.151 2020/06/12 19:29:19 mariano Exp $
##
## Home Page:
...
proc ::autopsf::autopsf_gui {} {
  ...
  grid $w.menubar.options -column 0 -row 0
  #pack $w.menubar.help -side right
  grid $w.menubar.help -column 1 -row 0
...
incr row
  ...
  grid [frame $w.input.basics] -column 0 -row 0
  grid [label $w.input.basics.mollable -text "Molecule: "] -column 0 -row 0
  grid [menubutton $w.input.basics.mol -height 1 -width 14 -textvar [namespace current]::molMenuText -menu $w.input.basics.mol.menu -relief raised] -column 1 -row 0
  menu $w.input.basics.mol.menu -tearoff no
  #pack $w.input.basics.mollable $w.input.basics.mol -expand 1 -fill x -side left  
  
  grid [label $w.input.basics.fplabel -text "Output basename: "] -column 2 -row 0
  grid [entry $w.input.basics.prefix  -textvariable [namespace current]::basename] -column 3 -row 0
#  pack $w.input.fplabel  -side top
  #pack $w.input.basics.fplabel $w.input.basics.prefix -side left
  #pack $w.input.basics 

  grid [labelframe $w.input.topo -bd 2 -relief ridge -text "Topology files" -padx 1m -pady 1m] -column 0 -row 1
  grid [frame $w.input.topo.list]
  grid [scrollbar $w.input.topo.list.scroll -command "$w.input.topo.list.list yview"] -column 1 -row 0 -sticky nsew
  grid [listbox $w.input.topo.list.list -activestyle dotbox -yscroll "$w.input.topo.list.scroll set" -font {tkFixed 9} \
      -width 55 -height 2 -setgrid 1 -selectmode browse -selectbackground $selectcolor \
      -listvariable [namespace current]::topfiles] -column 0 -row 0 -sticky nsew
  grid [frame $w.input.topo.list.buttons] -column 2 -row 0
  grid [button $w.input.topo.list.buttons.add -text "Add" -command [namespace code {
      set toptypes {
  ...
  grid [button $w.input.topo.list.buttons.delete -text "Delete" -command [namespace code {
  ...
  grid [button $w.input.next -text "Load input files" -command [namespace current]::afterinput_gui] -column 0 -row 2 -sticky nsew  
  #pack $w.input.topo
  #pack $w.input.next -fill x

  grid [labelframe $w.sels -relief ridge -text "Step 2: Mariano Selections to include in PSF/PDB" -padx 1m -pady 1m] -row $row -column 0 -columnspan 4 -sticky nsew
  set irow 0
...
#Frame for inspecting chains
  ...
  grid [label $w.chains.label -anchor w -font {tkFixed 9} -textvariable [namespace current]::formattext -relief flat] -column 0 -row 0 -sticky nsew 
  grid [frame $w.chains.list] -column 0 -row 1
  grid [scrollbar $w.chains.list.scroll -command "$w.chains.list.list yview"] -column 1 -row 0 -sticky nsew
  grid [listbox $w.chains.list.list -activestyle dotbox -yscroll "$w.chains.list.scroll set" -font {tkFixed 9} \
      -width 45 -height 4 -setgrid 1 -selectmode extended -selectbackground $selectcolor \
      -listvariable [namespace current]::chaintexts] -column 0 -row 0 -sticky nsew
  #pack $w.chains.label -side top -anchor w
  #pack $w.chains.list.list $w.chains.list.scroll -side left -fill y -expand 1
  grid [frame $w.chains.edits] -column 1 -row 1  
  grid [button $w.chains.edits.add -text "Add a new chain" -command [namespace current]::addchain_gui] -column 0 -row 0 -sticky nsew
  grid [button $w.chains.edits.edit -text "Edit chain" -command [namespace current]::editchain_gui] -column 0 -row 1 -sticky nsew
  grid [button $w.chains.edits.del -text "Delete chain" -command [namespace current]::delchain_gui] -column 0 -row 2 -sticky nsew
  grid [button $w.chains.finish -text "Create chains" -command [namespace current]::afterchains_gui] -column 0 -columnspan 2 -row 2 -sticky nsew
...
#Frame for patches
  ...
  grid [label $w.patches.label -anchor w -font {tkFixed 9} -textvar [namespace current]::patchformattext -relief flat -justify left] -column 0 -row 0 -sticky nsew
  grid [frame $w.patches.list] -column 0 -row 1
  grid [scrollbar $w.patches.list.scroll -command "$w.patches.list.list yview"] -column 1 -row 0 -sticky nsew
  grid [listbox $w.patches.list.list -activestyle dotbox -yscroll "$w.patches.list.scroll set" -font {tkFixed 9} -width 45 -height 4 -setgrid 1 -selectmode extended -selectbackground $selectcolor -listvariable [namespace current]::patchtexts] -column 0 -row 0 -sticky nsew
  #pack $w.patches.label -side top -anchor w
  #pack $w.patches.list.list $w.patches.list.scroll -side left -fill y -expand 1
  grid [frame $w.patches.edits] -column 1 -row 1
  grid [button $w.patches.edits.add -text "Add patch" -command [namespace current]::addpatch_gui] -column 0 -row 0 -sticky nsew 
  grid [button $w.patches.edits.del -text "Delete patch" -command [namespace current]::delpatch_gui] -column 0 -row 1 -sticky nsew 
  grid [button $w.patches.finish -text "Apply patches and finish PSF/PDB" -command [namespace current]::makepatches_gui] -column 0 -columnspan 2 -row 2 -sticky nsew
...
proc ::autopsf::sort_to_writepdb { molid selection outPutFileName} {
  ...
  foreach chain [lsort -unique [$selection get chain]] {
    set selectionAux [atomselect $molid "\(residue [lsort -unique -integer [$selection get residue]]\) and chain \"$chain\""]  
    set residueList [lsort -unique -integer -increasing  [$selectionAux get residue] ]

    set residList [$selectionAux get resid]
    
    ### Get the unsorted list of resIDs and check if re-ordering is needed
    array set x {}
    set res {}
    set needSort 0
    set size 0
    foreach e $residList {
        ### Check if the order of the residues is inverted, or if there is 
        ### a split residue, or if the resIDs are out of order
        if {!$needSort && $size > 1 && ([lindex $res 0] > [lindex $res 1] || [lindex $res end] > $e) } {
          set needSort 1
        }
    ...
    ### if there is nothing to do with this chain, set sel equal to chain
    if {$needSort == 0} {
      lappend sel $selectionAux  
      continue 
    }
...
#                mol addrep $currentMol
	...
	lappend fnamelist $newname
        set first 0; #if not set to 0 and the chain has only one residue, the loop fails
	set newfile 0
	set newseg 0
	set isrna 0
	set isdna 0
...
proc ::autopsf::fragment_chooser {} {
  ...
  grid [frame $w.frag]
  grid [labelframe $w.frag.docu -text "Info" -padx 2m -pady 2m]
  grid [label $w.frag.docu.1 -anchor w -textvariable ::autopsf::fchooserheader -font {-size 10 -weight bold }] -sticky nswe
  grid [label $w.frag.docu.2 -anchor w -text "Don't worry, there are several things you can do:"] -sticky nswe
  grid [label $w.frag.docu.3 -anchor w -text "1) Load an additional topology file that contains the missing information and rerun psfgen."] -sticky nswe
  grid [label $w.frag.docu.4 -anchor w -text "2) Delete the component from your system and rerun psfgen."] -sticky nswe
  grid [label $w.frag.docu.5 -anchor w -text "3) Parametrize the unknown components using Paratool."] -sticky nswe
  ...
  grid [labelframe $w.frag.topo -bd 2 -relief ridge -text "Unparametrized components" -padx 1m -pady 1m]
  grid [frame $w.frag.topo.list] -column 0 -row 0 -sticky nswe
  grid [scrollbar $w.frag.topo.list.scroll -command "$w.frag.topo.list.list yview"] -column 1 -row 0 -sticky nswe 
  grid [listbox $w.frag.topo.list.list -activestyle dotbox -yscroll "$w.frag.topo.list.scroll set" -font {tkFixed 9} \
      -width 40 -height 4 -setgrid 1 -selectmode browse -selectbackground $selectcolor \
      -listvariable ::autopsf::fragmentlist] -column 0 -row 0 -sticky nswe 
  grid [frame  $w.frag.topo.buttons] -column 0 -row 1 -sticky nswe
  grid [button $w.frag.topo.buttons.add -text "Parametrize"  -command [namespace code {
      variable unparsel
  ...
  grid [button $w.frag.topo.buttons.delete -text "Delete" -command [namespace code {
      variable tmpmolid
      variable unparsel
  ...
  grid [button $w.frag.rerun -text "Rerun psfgen" -command [namespace current]::rerunpsfgen]
