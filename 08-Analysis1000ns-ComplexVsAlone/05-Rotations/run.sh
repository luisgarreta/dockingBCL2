...
rotations-ALLP.py $DIR1 
rotations-ALLP.py $DIR2 

cp out-$DIR1/out-$DIR1.csv $OUT
cp out-$DIR2/out-$DIR2.csv $OUT

cmm="create-longtable-fromFiles.py $OUT"; echo $cmm; eval $cmm
cmm='plot-XY-MultiLine.R $OUT-LONG.csv "FRAME" "AXIS" "SYSTEM" "Rotations for complex vs. protein" "FRAME (ns)"'; echo $cmm; eval $cmm
